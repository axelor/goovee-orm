...
- **Compact Selection:** When `select` is empty or relational fields are selected with `true`, now returns only `id` and `version` fields for optimized queries (fixes #7).
